...
namespace JiaJiao.DAL
{
	...
	public partial class Class
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "Class"); 
		}
		...
		public JiaJiao.Model.Class GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Day,Time from Class ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public JiaJiao.Model.Class DataRowToModel(DataRow row)
		{
			JiaJiao.Model.Class model=new JiaJiao.Model.Class();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,Day,Time ");
