...
namespace JiaJiao.DAL
{
	...
	public partial class Student
	{
		...
		public bool Update(JiaJiao.Model.Student model)
		{
			...
			strSql.Append("StuAddress=@StuAddress,");
			strSql.Append("RoleId=@RoleId,");
			strSql.Append("CreateTime=@CreateTime,");
			strSql.Append("UpdteTime=@UpdteTime");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@StuName", SqlDbType.NVarChar,50),
					new SqlParameter("@StuEmail", SqlDbType.NVarChar,50),
		...
		/// 得到一个对象实体
		/// </summary>
		public JiaJiao.Model.Student GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,StuName,StuEmail,StuTel,StuAddress,RoleId,CreateTime,UpdteTime from Student ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public JiaJiao.Model.Student DataRowToModel(DataRow row)
		{
			JiaJiao.Model.Student model=new JiaJiao.Model.Student();
			if (row != null)
			{
				...
				if(row["StuAddress"]!=null)
				{
					model.StuAddress=row["StuAddress"].ToString();
				}
				...
				if(row["UpdteTime"]!=null && row["UpdteTime"].ToString()!="")
				{
					model.UpdteTime=DateTime.Parse(row["UpdteTime"].ToString());
				}
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,StuName,StuEmail,StuTel,StuAddress,RoleId,CreateTime,UpdteTime ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,StuName,StuEmail,StuTel,StuAddress,RoleId,CreateTime,UpdteTime ");
			strSql.Append(" FROM Student ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
